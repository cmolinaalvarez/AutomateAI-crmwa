-- Notify every newly assigned agent, including self-assignment.
--
-- Previously, assigning a conversation to yourself returned before inserting
-- the notification. After removing and reassigning a conversation, the sidebar
-- counter therefore stayed at zero even though the user owned new work.
...
BEGIN
  ...
  INSERT INTO notifications (
    ...
    CASE
      WHEN auth.uid() IS NOT NULL AND auth.uid() = NEW.assigned_agent_id
        THEN 'You assigned yourself a conversation with '
          || COALESCE(v_contact_name, 'a contact')
      ELSE COALESCE(v_actor_name, 'Someone') || ' assigned you a conversation with '
        || COALESCE(v_contact_name, 'a contact')
    END
  );
