...
describe('buildConversationContext', () => {
  ...
  it('represents an inbound voice note without inventing a transcript', async () => {
    const out = await buildConversationContext(
      fakeDb([
        { sender_type: 'customer', content_type: 'audio', content_text: null },
        { sender_type: 'assistant', content_type: 'audio', content_text: null },
      ]),
      'conv-1',
    )
    expect(out).toEqual([
      { role: 'user', content: '[Customer sent a voice message]' },
    ])
  })
})
